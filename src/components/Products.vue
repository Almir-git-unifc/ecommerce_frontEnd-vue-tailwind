      ...
      <main class="container mx-auto px-4">
         <header class="flex justify-between items-center mb-8">
            ...
            <nav class="flex space-x-4" aria-label="Product carousel controls">
               <!-- prevSlide aos 25:09min.  -->
               <button @click="prevSlide"  class="p-2 rounded-full bg-white shadow-md 
                     hover:bg-pink-100 text-pink-600 transition-colors" aria-label="Previous slide"
               > 
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
		               <g fill="none" stroke="#aa27a5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
			               <path stroke-dasharray="16" stroke-dashoffset="16" d="M19 12h-13.5">
				               <animate fill="freeze" attributeName="stroke-dashoffset" dur="0.2s" values="16;0" />
			               </path>
			               <path stroke-dasharray="10" stroke-dashoffset="10" d="M5 12l5 5M5 12l5 -5">
               ...
               <!-- nextSlide aos 25:09min. -->
               <button @click="nextSlide" class="p-2 rounded-full bg-white shadow-md 
                     hover:bg-pink-100 text-pink-600 transition-colors" aria-label="Next slide"
               >
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
		               <g fill="none" stroke="#aa27a5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
         ...
         <section class="relative overflow-hidden">
            ...
            > 
               <!-- v-for aos 26:22min.      -->
               <li  v-for="product in visibleProducts" :key="product.id"  class="bg-white 
               ...
               <!-- v-for aos 27:07min.     -->
                        <figcaption v-if="product.discount>0"
                           class="absolute top-3 right-3 bg-red-500 text-white text-xs 
                        ...
                        <button class="absolute top-3 left-3 p-2 bg-white rounded-full 
               ...
               <!-- v-for aos 28:22min.     -->
                     <section class="p-4">
                        <div class="flex items-center mb-2">
                           <Icon v-for="i in 5" :key="i" icon="line-md:star-alt-filled"                                 
                                width="16" height="16" :class="i <= Math.floor(product.rating)                                 
               ...
               <!-- v-for aos 29:13min.  -->
                        <footer class="flex items-center justify-between">
                           <div>
                              ...
                              <span v-if="product.discount > 0" class="text-sm text-gray-500 
                                   line-through ml-2"
                               >
               ...
               <!-- v-for aos 29:48min.    -->
                           <button class="p-2 bg-pink-950 rounded-full text-white  
                                hover:bg-pink-700 transition-colors" aria-label="Add to cart"
                           >
         ...
         <!-- v-for aos 30:30min.    -->
         <nav class="flex justify-center mt-6 space-x-2" aria-label="Carousel          
             pagination"
         >
            <button v-for="index in totalSlides" :key="index" @click="currentSlide = 
                index-1" :class="['w-3 h-3 rounded-full', currentSlide === index-1? 
                'bg-pink-950':'bg-gray-400']"
            >
            </button> 
         </nav>
...
<script setup>
  ...
  /*  Não coloque product0, product1, etc entre chaves */
   ...
   const handleResize = () =>{    /* Aos 31:10 minutos */
